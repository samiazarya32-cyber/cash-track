...
const TRANSLATIONS = {
  ti: {
    ...
    price: 'ዋጋ', quantity: 'ብዝሒ', search: 'በዚ ፈትሽ...', noData: 'መዝገብ የለን።',
    ...
    txInput: 'Transaction ID የእትው', submitTx: 'መለልይ ቁጽሪ ስደድ', active: 'ንጡፍ',
  ...
  am: {
    ...
    paymentTitle: 'የደንبهኝነት ክፍያ', premiumMsg: 'እባክዎ መተግبهሪያውን ለመጠቀም ክፍያ ይፈጽሙ።',
    txInput: 'የግብይት መታወቂያ (TXID) አስገባ', submitTx: 'መታወቂያ ላክ', active: 'ንቁ',
    aiBtn: 'በ AI መርምር', downloadReport: 'ሪፖርት አውርድ', scanBarcode: 'ባርኮد ስካን አድርግ',
...
const BUSINESS_TYPES = {
  ti: ['ሳሎን (Saloon)', 'ሱፐርማርኬት', 'ሚኒ ማርኬት', 'ባር (Bar)', 'ቤት መግቢ (Restaurant)', 'ስቱዲዮ (Videography)', 'ዱኳን (Shop)', 'ሆስፒتال', 'ፋርማሲ', 'ካልእ ቢዝነስ'],
...
export default function App() {
  ...
  const [inventory, setInventory] = useState(() => JSON.parse(localStorage.getItem('ct_inventory_v3')) || []);
  const [sales, setSales] = useState(() => JSON.parse(localStorage.getItem('ct_sales_v3')) || []);
  ...
  // Custom Smarter PWA Banner State (Forced to guide users clearly)
  ...
  useEffect(() => { localStorage.setItem('ct_inventory_v3', JSON.stringify(inventory)); }, [inventory]);
  useEffect(() => { localStorage.setItem('ct_sales_v3', JSON.stringify(sales)); }, [sales]);
  ...
  const handleRecordSale = (e) => {
    ...
    const finalCost = selectedProduct ? selectedProduct.cost : finalPrice * 0.7;
  ...
  // Helper instruction trigger for manual install guide
  const showManualInstallGuide = () => {
    alert("How to Install Cash Track:\n\n🤖 Android (Chrome): Tap the 3 dots in the top-right corner and select 'Add to Home screen' or 'Install app'.\n\n🍏 iPhone (Safari): Tap the 'Share' icon (square with arrow) at the bottom, then scroll down and tap 'Add to Home Screen'.");
  };
  ...
  return (
    <div style={{ background: '#F4F6F8', minHeight: '100vh', paddingBottom: '80px', fontFamily: 'system-ui, sans-serif', boxSizing: 'border-box' }}>
      ...
      {/* FIXED GUIDING PWA SMART BANNER */}
      {showPwaPopUp && (
        <div style={{ background: '#2A9D8F', color: '#FFF', padding: '12px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '14px', boxShadow: '0 4px 6px rgba(0,0,0,0.05)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <Smartphone size={18} />
            <span>📲 <strong>{t.installApp}፦</strong> ነዛ ኣፕ ኣብ ስልክኻ ንምጽዓን "Install" ንበል እሞ መምርሒ ርአ።</span>
          </div>
          <div style={{ display: 'flex', gap: '12px' }}>
            <button onClick={showManualInstallGuide} style={{ background: '#1E293B', color: '#FFF', border: 'none', padding: '6px 12px', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold' }}>Install</button>
            <button onClick={() => setShowPwaPopUp(false)} style={{ background: 'transparent', border: 'none', color: '#FFF', cursor: 'pointer' }}><X size={18} /></button>
          </div>
        </div>
      )}

      {/* MAIN CONTAINER: Dynamic Flex Grid Setup to Fix Laptop/Tablet Views */}
      <main style={{ maxWidth: '1200px', margin: '0 auto', padding: '20px' }}>
        <div className="responsive-grid">
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px' }}>
                  ...
                  <div style={{ background: '#FFF', padding: '20px', borderRadius: '12px', boxShadow: '0 2px 5px rgba(0,0,0,0.05)', borderLeft: '6px solid #E76F51' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', color: '#64748B', fontSize: '14px', marginBottom: '6px' }}>
                      <span>{t.profit}</span> <DollarSign color="#E76F51" />
                    </div>
                ...
                <div style={{ background: '#FFF', padding: '20px', borderRadius: '12px', boxShadow: '0 2px 5px rgba(0,0,0,0.05)' }}>
                  ...
                  {isScanning && (
                    <div style={{ background: '#000', borderRadius: '8px', padding: '10px', marginBottom: '16px', color: '#FFF', textAlign: 'center' }}>
                      <video ref={videoRef} autoPlay playsInline style={{ width: '100%', maxHeight: '200px', objectFit: 'cover', borderRadius: '6px' }} />
                      <div style={{ display: 'flex', justifyContent: 'center', gap: '10px', marginTop: '10px' }}>
                        <button onClick={triggerMockScanSuccess} style={{ background: '#2A9D8F', border: 'none', color: '#FFF', padding: '4px 10px', borderRadius: '4px', cursor: 'pointer' }}>Simulate Scan</button>
                        <button onClick={stopCameraScan} style={{ background: '#DC3545', border: 'none', color: '#FFF', padding: '4px 10px', borderRadius: '4px', cursor: 'pointer' }}>Close</button>
                      </div>
                    </div>
                  )}
                  ...
                  <form onSubmit={handleRecordSale} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    <div>
                      <select value={saleItemName} onChange={(e) => setSaleItemName(e.target.value)} style={{ width: '100%', padding: '10px', borderRadius: '6px', border: '1px solid #CBD5E1' }} required>
                        <option value="">{t.selectItem}</option>
                        {inventory.map(item => <option key={item.id} value={item.name}>{item.name} (In stock: {item.qty})</option>)}
                      </select>
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                      <input type="number" value={saleQty} onChange={(e) => setSaleQty(e.target.value)} style={{ width: '100%', padding: '10px', borderRadius: '6px', border: '1px solid #CBD5E1' }} placeholder={t.quantity} required />
                      <input type="number" value={saleCustomPrice} onChange={(e) => setSaleCustomPrice(e.target.value)} style={{ width: '100%', padding: '10px', borderRadius: '6px', border: '1px solid #CBD5E1' }} placeholder="Override Price (Optional)" />
                    </div>
          ...
          {currentTab === 'inventory' && (
            <div style={{ gridColumn: '1 / -1', display: 'flex', flexDirection: 'column', gap: '20px' }}>
              <div style={{ background: '#FFF', padding: '20px', borderRadius: '12px', boxShadow: '0 2px 5px rgba(0,0,0,0.05)' }}>
                ...
                <form onSubmit={handleAddProduct} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '12px', alignItems: 'end' }}>
                  <input type="text" value={prodName} onChange={(e) => setProdName(e.target.value)} style={{ padding: '10px', borderRadius: '6px', border: '1px solid #CBD5E1' }} placeholder={t.itemName} required />
                  <input type="number" value={prodCost} onChange={(e) => setProdCost(e.target.value)} style={{ padding: '10px', borderRadius: '6px', border: '1px solid #CBD5E1' }} placeholder={`${t.buyingPrice} (${currency})`} required />
                  <input type="number" value={prodPrice} onChange={(e) => setProdPrice(e.target.value)} style={{ padding: '10px', borderRadius: '6px', border: '1px solid #CBD5E1' }} placeholder={`${t.sellingPrice} (${currency})`} required />
                  <input type="number" value={prodQty} onChange={(e) => setProdQty(e.target.value)} style={{ padding: '10px', borderRadius: '6px', border: '1px solid #CBD5E1' }} placeholder={t.quantity} required />
          ...
          {/* AI HISTORY REPORTS PANEL */}
          {currentTab === 'history' && (
            <div style={{ gridColumn: '1 / -1', background: '#FFF', padding: '20px', borderRadius: '12px', boxShadow: '0 2px 5px rgba(0,0,0,0.05)' }}>
          ...
          {currentTab === 'profile' && (
            <div style={{ gridColumn: '1 / -1', background: '#FFF', padding: '20px', borderRadius: '12px', boxShadow: '0 2px 5px rgba(0,0,0,0.05)', maxWidth: '500px', margin: '0 auto', width: '100%' }}>
              ...
              <div style={{ border: '1px solid #E2E8F0', padding: '16px', borderRadius: '8px', background: '#F8FAFC' }}>
                ...
                <p style={{ fontSize: '13px', color: '#475569', lineHeight: '1.4' }}>
                  Send subscription funds to Mobile Money number:<br />
                  <strong style={{ fontSize: '15px', color: '#1E293B' }}>00256743227053</strong><br />
                  Registered Owner: <strong>Yohannes Kiflay Zera</strong>
                </p>
                <form onSubmit={handleTxSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginTop: '12px' }}>
                ...
                {paymentStatus && <p style={{ fontSize: '13px', color: '#2A9D8F', marginTop: '10px' }}>{paymentStatus}</p>}
      ...
      {/* FOOTER TASKBAR CONTROLS BAR */}
      <nav style={{ position: 'fixed', bottom: 0, left: 0, right: 0, height: '65px', background: '#1E293B', borderTop: '1px solid #334155', display: 'flex', justifyContent: 'space-around', alignItems: 'center', zIndex: 1000 }}>
        ...
        <button onClick={showManualInstallGuide} style={{ background: 'transparent', border: 'none', color: '#E76F51', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px', cursor: 'pointer', fontSize: '12px' }}>
          <Smartphone size={20} /> <span>{t.installApp}</span>
        </button>
      </nav>

      {/* Embedded Global Styles Rule Definition to perfect Laptop Layout Structure Viewports */}
      <style>{`
        .responsive-grid {
          display: grid;
